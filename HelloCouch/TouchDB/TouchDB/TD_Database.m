...
NSString* const TD_DatabaseBeginTransactionNotification     = @"TD_DatabaseBeginTransactionNotification";
NSString* const TD_DatabaseRollbackTransactionNotification  = @"TD_DatabaseRollbackTransactionNotification";
NSString* const TD_DatabaseCommitTransactionNotification    = @"TD_DatabaseCommitTransactionNotification";
NSString* const TD_DatabaseTransactionFailedNotification    = @"TD_DatabaseTransactionFailedNotification";

@implementation TD_Database
{
    struct {
        unsigned int beginTransaction: 1;
        unsigned int commitTransaction: 1;
        unsigned int rollbackTransaction: 1;
        unsigned int endTransactionFaield: 1;
...
- (BOOL) beginTransaction {
    ...
    {
        [[NSNotificationCenter defaultCenter] postNotificationName:TD_DatabaseBeginTransactionNotification object:self userInfo:@{ @"TD_Database" : self }];
    }
    if (_notifyDelegateOnNestedTransactionEvents || _transactionLevel == 0)
    {
        [self databaseBeginTransaction:_transactionLevel];
    }
    ++_transactionLevel;
...
- (BOOL) endTransaction: (BOOL)commit {
    ...
    if (commit) {
        ...
        {
            if (_transactionLevel == 1) // 1 because it has not been decreased
            {
                [[NSNotificationCenter defaultCenter] postNotificationName:TD_DatabaseTransactionFailedNotification object:self userInfo:@{ @"TD_Database" : self }];
            }
            if (_transactionLevel == 1 || _notifyDelegateOnNestedTransactionEvents)
            {
                [self databaseEndTransactionFailed:_transactionLevel-1];
            }
            return NO;
        }
    }
    ...
    if (_transactionLevel == 0)
    {
        if (commit)
        {
            [[NSNotificationCenter defaultCenter] postNotificationName:TD_DatabaseCommitTransactionNotification object:self userInfo:@{ @"TD_Database" : self }];
        }
        else
        {
            [[NSNotificationCenter defaultCenter] postNotificationName:TD_DatabaseRollbackTransactionNotification object:self userInfo:@{ @"TD_Database" : self }];
        }
    }
    if (_transactionLevel == 0 || _notifyDelegateOnNestedTransactionEvents)
    {
        if (commit)
        {
            [self databaseCommitTransaction:_transactionLevel];
        }
        else
        {
            [self databaseRollbackTransaction:_transactionLevel];
        }
    }
    return YES;
}
...
- (void)setDelegate:(id<TD_DatabaseDelegate>)delegate
{
    _delegate = delegate;
    
    _delegateFlags.beginTransaction = _delegate && [(id)_delegate respondsToSelector:@selector(database:beginTransaction:)];
    _delegateFlags.commitTransaction = delegate && [(id)_delegate respondsToSelector:@selector(database:commitTransaction:)];
    _delegateFlags.rollbackTransaction = _delegate && [(id)_delegate respondsToSelector:@selector(database:rollbackTransaction:)];
    _delegateFlags.endTransactionFaield = _delegate && [(id)_delegate respondsToSelector:@selector(database:endTransactionFailed:)];
    _delegateFlags.didChange = _delegate && [(id)_delegate respondsToSelector:@selector(database:didChange:)];
}
- (void)databaseBeginTransaction:(NSInteger)txLevel
{
    if (_delegateFlags.beginTransaction)
    {
        [_delegate database:self beginTransaction:txLevel];
    }
}
- (void)databaseCommitTransaction:(NSInteger)txLevel
{
    if (_delegateFlags.commitTransaction)
    {
        [_delegate database:self commitTransaction:txLevel];
    }
}
- (void)databaseRollbackTransaction:(NSInteger)txLevel
{
    if (_delegateFlags.rollbackTransaction)
    {
        [_delegate database:self rollbackTransaction:txLevel];
    }
}
- (void)databaseEndTransactionFailed:(NSInteger)txLevel
{
    if (_delegateFlags.endTransactionFaield)
    {
        [_delegate database:self endTransactionFailed:txLevel];
    }
}
- (void)databaseDidChange:(NSDictionary *)change
{
    if (_delegateFlags.didChange)
    {
        [_delegate database:self didChange:change];
    }
}
@end
